...
export default function WaiveFeeModal({ isOpen, onClose, group, onSuccess }: WaiveFeeModalProps) {
  ...
  const handleWaive = async () => {
    ...
    try {
      ...
      const successKey = waivedCount === 1 ? 'fees.waiveSuccess' : 'fees.waiveSuccessPlural';
      toast.success(
        `✅ ${t(successKey, { amount: `$${totalWaived.toFixed(2)}`, customer: customerName, staff: waivedBy, count: waivedCount })}`,
        { duration: 5000 }
      );
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title={`💰 ${t('fees.waiveModalTitle')}`}>
      <div className="space-y-4">
        {/* Customer Info */}
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <p className="text-sm text-gray-600 mb-1">{t('fees.waivingFeesFor')}</p>
          <p className="text-lg font-bold text-gray-900">{customerName}</p>
          <p className="text-sm text-gray-600">📮 {t('fees.mailbox')}: {group.contact.mailbox_number || 'N/A'}</p>
        </div>

        {/* Fee Summary */}
        <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-gray-700">{t('fees.totalFeesToWaive')}</span>
          ...
          <div className="mt-3 space-y-1">
            <p className="text-xs font-semibold text-gray-700 uppercase">{t('fees.packages')} ({pendingPackages.length}):</p>
            {pendingPackages.map(pkg => (
              <div key={pkg.mail_item_id} className="text-xs text-gray-600 ml-2">
                • {t('fees.day')} {pkg.packageFee?.days_charged || 0} - ${(pkg.packageFee?.fee_amount || 0).toFixed(2)}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            {t('fees.reasonForWaiving')} <span className="text-red-500">*</span>
          </label>
          <textarea
            value={reason}
          ...
          <p className="text-xs text-gray-500 mt-1">
            {t('fees.charactersCount', { current: reason.length, max: 500, min: 5 })}
          </p>
        </div>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-900 mb-2">
            {t('fees.whoIsWaiving')} <span className="text-red-500">*</span>
          </label>
          ...
          {!waivedBy && (
            <p className="mt-1 text-xs text-red-600">{t('fees.selectWhoIsWaiving')}</p>
          )}
        </div>
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
          <p className="text-xs text-yellow-800">
            ⚠️ <strong>{t('fees.waiveWarning')}</strong> {t('fees.waiveWarningDetail')}
          </p>
        </div>
        ...
        <div className="flex gap-2 justify-end pt-2">
          ...
          >
            {saving ? t('fees.waivingInProgress') : t('fees.waiveAmount', { amount: `$${group.totalFees.toFixed(2)}` })}
          </button>
        </div>
      </div>
    </Modal>
